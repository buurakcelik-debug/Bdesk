...
hbb_name = 'bdesk' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2('pip3 install -r requirements.txt')
        system2(f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/*.exe')
    os.chdir('../..')
    if os.path.exists('./bdesk_portable.exe'):
        ...
        os.rename('./target/release/rustdesk-portable-packer.exe',
                  './bdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/bdesk_portable.exe')
    os.rename('./bdesk_portable.exe', f'./bdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/bdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        system2('mv ./flutter/build/windows/runner/Release/*.exe ./flutter/build/windows/runner/Release/bdesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                './flutter/build/windows/runner/Release/bdesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf ./flutter/build/windows/runner/Release/bdesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/bdesk-{version}-win7-install.exe')
        system2('mv ../../{res_dir}/bdesk-{version}-win7-install.exe ../..')
